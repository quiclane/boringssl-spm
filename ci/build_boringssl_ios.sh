...
build_one(){
  ...
  local dir="$OUT/$name"
  cmake -S "$UPDIR" -B "$dir" -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_SYSTEM_NAME=iOS -DCMAKE_OSX_SYSROOT="$sysroot" -DCMAKE_OSX_ARCHITECTURES="$arch" -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DOPENSSL_NO_ASM=OFF -DBUILD_SHARED_LIBS=OFF -DBUILD_TESTING=OFF -DCMAKE_SKIP_INSTALL_RULES=ON -DINSTALL_ENABLED=OFF -DBUILD_TOOL=OFF
